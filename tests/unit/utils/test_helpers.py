...
class TestFormatResponse:
    ...
    def test_simple_response(self):
        ...
        assert formatted == {"result": "Test result", "success": True}
    ...
    def test_detailed_response(self):
        ...
        # Check key fields
        assert formatted["result"] == "Test result"
        assert formatted["success"] is True
        assert formatted["details"]["best_response"]["provider"] == "OpenAI"
        assert formatted["details"]["method"] == "select"
        assert formatted["details"]["judge_response"] == "Judge explanation"
        assert formatted["details"]["weights"] == [0.7, 0.3]
        # Explanation field will be None since it wasn't in the input
        assert "explanation" in formatted
        assert formatted["explanation"] is None

    def test_response_with_explanation(self):
        """Test formatting a response with explanation field."""
        data = {
            "result": "Test result",
            "success": True,
            "explanation": "This is an explanation of the judge's decision",
            "method": "select",
            "best_response": {"provider": "OpenAI"},
        }
        ...
        # Explanation should be present at both top level and in details
        assert (
            formatted["explanation"] == "This is an explanation of the judge's decision"
        )
        assert (
            formatted["details"]["explanation"]
            == "This is an explanation of the judge's decision"
        )
        assert formatted["result"] == "Test result"
        assert formatted["success"] is True
    ...
    def test_missing_fields(self):
        ...
        # Explanation field should be present but None
        assert "explanation" in formatted
        assert formatted["explanation"] is None
