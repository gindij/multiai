...
def parse_args():
    ...
    compare_parser.add_argument(
        "--blend", "-b", action="store_true", help="Blend responses instead of selecting one"
    )
    compare_parser.add_argument(
    ...
    compare_parser.add_argument(
        "--openai", help=f"OpenAI model to use (default: {AVAILABLE_MODELS['openai']['default']})"
    )
    compare_parser.add_argument(
        "--anthropic", help=f"Anthropic model to use (default: {AVAILABLE_MODELS['anthropic']['default']})"
    )
    compare_parser.add_argument(
        "--gemini", help=f"Google Gemini model to use (default: {AVAILABLE_MODELS['gemini']['default']})"
    )
    ...
    # Setup command
    setup_parser = subparsers.add_parser("setup", help="Setup API keys and configuration")
