...
@pytest.fixture
def mock_models():
    with patch('multi_ai.services.comparator.OpenAIModel') as mock_openai, \
         patch('multi_ai.services.comparator.AnthropicModel') as mock_anthropic, \
         patch('multi_ai.services.comparator.GeminiModel') as mock_gemini, \
         patch('multi_ai.services.comparator.Judge') as mock_judge:
...
@pytest.fixture
def comparator():
    with patch('multi_ai.models.openai_model.OpenAIModel') as mock_openai, \
         patch('multi_ai.models.anthropic_model.AnthropicModel') as mock_anthropic, \
         patch('multi_ai.models.gemini_model.GeminiModel') as mock_gemini, \
         patch('multi_ai.services.judge.Judge') as mock_judge:
...
class TestComparator:
    ...
    async def test_compare_default_models(self, comparator):
        ...
        anthropic_mock.query.assert_called_once_with("Test prompt", ANTHROPIC_DEFAULT_MODEL)
    ...
    async def test_compare_custom_models(self, comparator):
        ...
        anthropic_mock.query.assert_called_once_with("Test prompt", "claude-3-opus-20240229")
