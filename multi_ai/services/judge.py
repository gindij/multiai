...
class Judge:
    ...
    async def evaluate(
        ...
        anonymized_responses, provider_map = self._anonymize_responses(successful_responses)
        ...
        if self.blend_responses:
            ...
            # Parse selected response index
            best_idx = self._parse_selected_index(judge_response["response"])
    ...
    def _anonymize_responses(self, responses: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[int, Tuple[int, str, str]]]:
        """
    ...
    def _create_evaluation_prompt(
        ...
        if blend_mode:
            prompt_parts.append(
                "\nYOUR RESPONSE FORMAT: Assign a weight between 0 and 10 to each response based on overall quality."
            )
            prompt_parts.append(
                'Reply using only this JSON format: {"weights": [X, Y, Z]}'
            )
            prompt_parts.append(
                "Where X, Y, Z are numbers between 0 and 10 representing the quality of each response."
            )
            prompt_parts.append("CRITICAL: Provide only the JSON object, nothing else.")
        else:
            prompt_parts.append(
                "\nYOUR RESPONSE FORMAT: Identify the number of the best response."
            )
            prompt_parts.append(
                "Reply with only the number, for example: '2'"
            )
            prompt_parts.append(
                "CRITICAL: Provide only the number, no explanation or other text."
            )

        return "\n".join(prompt_parts)

    def _parse_selected_index(self, judge_text: str) -> Optional[int]:
        """Parse the judge's response to determine the best model response index."""
    ...
    def _parse_weights(self, judge_text: str, num_responses: int) -> List[float]:
        """Parse the weights from the judge's response."""
        try:
        ...
        return [1.0 / num_responses] * num_responses
